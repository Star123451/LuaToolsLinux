...
#
# ==================================================
#   Millennium Fix & Install Script (v4 - Conflict Fix)
# ==================================================
...
# --- STEP 1: BRUTAL CLEANUP (DATABASE + FILES) ---
uninstall_brutal() {
    ...
    # 2. REMOVE FROM SYSTEM DATABASE (Fixes the conflict error)
    log "-> Removing packages from database to avoid conflicts..."
    sudo pacman -Rns millennium millennium-git --noconfirm 2>/dev/null

    # 3. Restore Steam startup scripts
    ...
    log "[ok] System is now fully clean (DB and Files)."
}

# --- STEP 2: OFFICIAL POST_INSTALL LOGIC ---
post_install() {
    ...
    for steam_path in "${HOME}/.steam/steam" "${HOME}/.local/share/Steam" "${HOME}/.var/app/com.valvesoftware.Steam/.local/share/Steam"; do
        ...
        if [ -d "${steam_path}/ubuntu12_32" ]; then
            # Check for the correct library name depending on the install method
...
main() {
    ...
    # 1. Clean the database conflict and files
    uninstall_brutal

    # 2. Arch/CachyOS Installation
    installed_aur=false
    if [ -f /etc/arch-release ] || [ -f /etc/cachyos-release ]; then
        log "\n[+] Arch-based system detected."
        # Now that conflicts are gone, we install millennium-git
        if command -v paru >/dev/null; then
            log "[+] Installing millennium-git via paru..."
            paru -S millennium-git --noconfirm && installed_aur=true
        elif command -v yay >/dev/null; then
            log "[+] Installing millennium-git via yay..."
            yay -S millennium-git --noconfirm && installed_aur=true
        fi
    fi

    # 3. Manual Fallback
    if [ "$installed_aur" = false ]; then
        log "\n[!] AUR installation failed or not Arch. Falling back to manual..."
        target="linux-x86_64"
    ...
    # 4. Final Post-Install (The "Final Part")
